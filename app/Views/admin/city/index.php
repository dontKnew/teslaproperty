    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <a href="<?= base_url().route_to("admin/city/add") ?>" class="btn btn-info">
            ...
            <div class="card-body">
                ...
                <?php if ($data): ?>
                    <?php if($pager['pagination']['hasMore']): ?>
                        <form action="<?= base_url().route_to("admin/city") ?>" method="post" >
                    ...
                    <div class="table-responsive">
                        <table class="table table-head-bg-primary table-striped table-hover text-center">
                            ...
                            <?php $i=1; foreach ($data as $value): ?>
                                <tr>
                                    <td><?= $i++ ?></td>
                                    <td><?= esc($value['state_name']) ?></td>
                                    <td><?= esc($value['name']) ?></td>
                                    <td><?= esc($value['page_title']) ?></td>
                                    <td>
                                        <a onclick="return confirm('Are you sure want to remove city ?')" href="<?= base_url(). route_to("admin/city/delete", $value['id']) ?>"
                                           class="btn btn-sm btn btn-rounded btn-danger my-1">
                                            <i class="la la-trash"></i> Delete
                                        </a>
                                        <a href="<?= base_url() . route_to("admin/city/update" , $value['id']) ?>"
